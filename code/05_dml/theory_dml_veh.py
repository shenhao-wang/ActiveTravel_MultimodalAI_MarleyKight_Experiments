"""
Theory-Driven Scalar DML — Job C: RF/GB × Full/NoVeh
22 features × 4 specs × B=500. Resume-safe.
Output: theory_dml_veh_results.csv
"""
...
# ── SPECS: Full vs No-Vehicle controls ───────────────────────
W_FULL_COLS  = INDIVIDUAL_CONFOUNDERS + ACS_FEATURES
W_NOVEH_COLS = [c for c in W_FULL_COLS if c != 'num_veh']
W_full  = df[[c for c in W_FULL_COLS if c in df.columns]].values.astype(float)
W_noveh = df[[c for c in W_NOVEH_COLS if c in df.columns]].values.astype(float)
print(f'W_full:  {W_full.shape[1]} features')
print(f'W_noveh: {W_noveh.shape[1]} features')
...
SPECS = {
    ...
    'RF_NoVeh': (clf_rf, reg_rf, W_noveh),
    'GB_Full':  (clf_gb, reg_gb, W_full),
    'GB_NoVeh': (clf_gb, reg_gb, W_noveh),
}


# ── 2. RUN 22 x 4 = 88 DML ESTIMATES ─────────────────────────
PARTIAL_PATH = 'theory_dml_veh_partial.csv'
completed = set()
results = []
...
# hypothesis family; pooling across RF/GB/Full/NoVeh would over-correct.
...
out.to_csv('theory_dml_veh_results.csv', index=False)
print(f'Saved: theory_dml_veh_results.csv')
...
for feat in ZONING_FEATURES:
    ...
    for spec_name in ['RF_Full', 'RF_NoVeh', 'GB_Full', 'GB_NoVeh']:
...
pivot.to_csv('theory_dml_veh_pivot.csv', index=False)
print(f'Saved: theory_dml_veh_pivot.csv')
print('\n✓ Complete.')
